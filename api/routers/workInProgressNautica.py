...
# Get all by username
@router.get("/user/{username}", response_model=List[IWorkInProgressNauticaRead])
def read_workinprogress_by_user(username: str, db: Session = Depends(get_db)):
    statement = select(WorkInProgressNautica).where(WorkInProgressNautica.completato_da_user == username)
    results = db.exec(statement).all()
    if not results:
